...
set SNLIB=E:\tools\demo\bug180529a
PATH=%SDCC_HOME%;%SDCC_HOME%\bin;%GPUTILS_HOME%\gpasm;%GPUTILS_HOME%\gplink


::set GPASM_ARG=-p p16f87 -I"%GPUTILS_HOME%\header"
set GPLINK_ARG=-s %SNLIB%\share\lkr\7333.lkr ^
%SNLIB%\share\lib\7511.lib ^
-w -m -c

set SDCC_ARG=-V --verbose --use-non-free -mmc32 -p7333 ^
-I"%SNLIB%\share\include" ^
-Wa"-I%SNLIB%\share\header" ^
-Wl"-s%SNLIB%\share\lkr\7333.lkr" ^
